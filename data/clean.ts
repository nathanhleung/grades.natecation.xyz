 ...
 */
async function parseAndIndexGrades(): Promise<[Row[], InstructorIndex, SubjectIndex]> {
    const rows: Row[] = [];
    const instructorIndex: InstructorIndex = {};
    const subjectIndex: SubjectIndex = {};

    const parser = fs.createReadStream(path.resolve(__dirname, 'grades.csv'))
...
async function main() {
    ...
    await Promise.all([
        fs.promises.writeFile(path.resolve(generatedDataDir, 'rows.json'), JSON.stringify(rows), 'utf-8'),
        fs.promises.writeFile(path.resolve(generatedDataDir, 'instructor-index.json'), JSON.stringify(instructorIndex), 'utf-8'),
        fs.promises.writeFile(path.resolve(generatedDataDir, 'subject-index.json'), JSON.stringify(subjectIndex), 'utf-8'),
    ]);
